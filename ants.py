...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3A
        Place.add_insect(self, insect) #self refers to?
        if insect.watersafe == False:
            insect.reduce_armor(insect.armor) #not need a self?
        # END Problem 3A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.real == False:
            ...
            while place_checker != None:
                if place_checker.ant != None:
                    if place_checker.ant not in self.doubled:
                        place_checker.ant.damage = place_checker.ant.damage * 2
                        self.doubled.append(place_checker.ant)
                    if place_checker.ant.container == True and place_checker.ant.ant != None and place_checker.ant.ant not in self.doubled:
                        place_checker.ant.ant.damage = place_checker.ant.ant.damage * 2
                        self.doubled.append(place_checker.ant.ant)
...
def make_slow(action):
    ...
    def slowed_action(colony):
        if colony.time % 2 == 0:
            self.action(colony) #idk how to call the action function of bee from here
        else:
            return
    return slowed_action #bee.slowed_action(colony)
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    def application(effect, bee, duration):
        nonlocal counter
        if counter < duration:
            ...
            counter = 0
    application(effect, bee, duration)
    #store old value set the action method back to old after duration
    #for the first five times i call this method do this
    #
    # END Problem EC
...
class SlowThrower(ThrowerAnt):
    ...
    def action(self,colony):
        target = self.nearest_bee(colony.hive)
...
class StunThrower(ThrowerAnt):
    ...
    def action(self,colony):
        apply_effect(make_stun, self.nearest_bee(colony.hive), 1)
